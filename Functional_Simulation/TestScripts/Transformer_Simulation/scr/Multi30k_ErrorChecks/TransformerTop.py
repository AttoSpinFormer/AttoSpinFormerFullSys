...
# Description:   Verification script for a **language model** mapped onto the MESO IMC architecture.
#
...
Note: The module evaluates the fidelity and performance impact of mapping **Transformer attention layers** onto a 
      MESO IMC infrastructure (128 x 16 MESO arrays).
"""
...
from Attention import (
	append_attention_error_summary_csv,
	print_attention_error_summary,
	reset_attention_error_stats,
	set_attention_error_phase,
	set_attention_error_tracking,
)
...
ATTENTION_ERROR_CSV_PATH = os.path.join('saved', 'attention_error_by_epoch.csv')
...
def evaluate_inference(model, iterator, criterion, collect_attention_errors=True, collect_generation_attention_errors=False):
	"""Evaluate model using inference mode (autoregressive generation)."""
	model.eval()
	set_attention_error_tracking(model, collect_attention_errors)
	epoch_loss = 0
	all_hypotheses = []
	all_references = []
	...
	with torch.no_grad():
		for i, batch in enumerate(iterator):
			...
			if collect_attention_errors and not collect_generation_attention_errors:
				set_attention_error_tracking(model, False)
				try:
					...
					set_attention_error_tracking(model, True)
			else:
				for _ in range(max_len - 1):
					output = model(src, decoded_trg) # [batch_size, current_len, voc_size]
					next_token = output[:, -1, :].argmax(dim=-1).unsqueeze(1) # [batch_size, 1]
...
def run(total_epoch, best_loss, start_epoch=0, train_losses=None, test_losses=None, bleus=None):
	...
	if mode == 0:
		...
		print(f"Chosen per cycle resolution of the input applied through the CMOS gate for current run: {cycle_res} bits.")
		print(f"Chosen device variability for current run: {var*100}%.")
	...
	for step in range(start_epoch, total_epoch):
		...
		# Collect fresh MESO-vs-CMOS attention error statistics for this epoch.
		# Training and validation are kept in separate phases.
		reset_attention_error_stats(model)
		set_attention_error_tracking(model, True)

		# 1) Train for the epoch and print/append training attention error
		#    before validation starts.
		set_attention_error_phase(model, "train")
		train_loss = train(model, train_iter, optimizer, criterion, clip)

		print("\n=== Training Attention Error Summary: Before Validation ===")
		print_attention_error_summary(model, epoch=current_epoch, phase="train")
		append_attention_error_summary_csv(
			model,
			csv_path=ATTENTION_ERROR_CSV_PATH,
			epoch=current_epoch,
			phase="train",
		)
		print(f"Training attention error summary appended to: {ATTENTION_ERROR_CSV_PATH}")
		print("==========================================================\n")

		# 2) Validate and then print/append validation attention error.
		set_attention_error_phase(model, "valid")
		valid_loss, bleu = evaluate_inference(
			model,
			valid_iter,
			criterion,
			collect_attention_errors=True,
			collect_generation_attention_errors=False,
		)
		end_time = time.time()

		print("\n=== Validation Attention Error Summary: After Validation ===")
		print_attention_error_summary(model, epoch=current_epoch, phase="valid")
		append_attention_error_summary_csv(
			model,
			csv_path=ATTENTION_ERROR_CSV_PATH,
			epoch=current_epoch,
		...
		print(f"Validation attention error summary appended to: {ATTENTION_ERROR_CSV_PATH}")
		print("===========================================================\n")
		set_attention_error_phase(model, None)
		...
		if current_epoch % 10 == 0:  # Every 10 epochs
			cleanup_old_checkpoints('saved', keep_best_n=5)


		print(f'Epoch: {current_epoch} | Time: {epoch_mins}m {epoch_secs}s')
...
if __name__ == '__main__':
	...
	test_loss, bleu = evaluate_inference(model, test_iter, criterion, collect_attention_errors=False)
