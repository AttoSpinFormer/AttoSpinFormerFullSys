...
class LayerNorm(nn.Module):
	...
	def forward(self,x):
		#the original code is not for running mean. Again, it is simple layer normalization that transformers use. However, for typical CNNs, we can use running mean and running var. 
		#However, the updates are only during training for batch norm and hence, we need to check if its training or not.
